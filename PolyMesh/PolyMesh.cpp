...
PolyMesh *PolyMesh::GenerateNormals()
{
	...
	for(VertexIter v_it = this->vertices_begin(); v_it != this->vertices_end(); ++v_it)
	{
		...
		for (VertexFaceIter vf_it = this->vf_begin(v_it.handle()); vf_it != this->vf_end(v_it.handle()); ++vf_it)
		{
		...
		// Set normal
		this->set_normal(v_it.handle(), normal);
	}

	return this;
}

PolyMesh *PolyMesh::LoopSubdivideP()
{
	int count;
	...
	// Old mesh handle data	
	EdgeHandle *oldEdgeHandles = (EdgeHandle *)calloc(edges, sizeof(EdgeHandle));
	VertexHandle *oldVertexHandles = (VertexHandle *)calloc(vertices, sizeof(VertexHandle));
	FaceHandle *oldFaceHandles = (FaceHandle *)calloc(faces, sizeof(FaceHandle));

	// Populate old mesh handle data. This part cannot be parallelized, due to
	// the serial nature of iterators. However, there might be another way...
	count = 0;
	for (EdgeIter e_it = edges_begin(); e_it != edges_end(); ++e_it)
	{
		oldEdgeHandles[count++] = e_it.handle();
	}
	count = 0;
	for (VertexIter v_it = vertices_begin(); v_it != vertices_end(); ++v_it)
	{
		oldVertexHandles[count++] = v_it.handle();
	}
	count = 0;
	for (FaceIter f_it = faces_begin(); f_it != faces_end(); ++f_it)
	{
		oldFaceHandles[count++] = f_it.handle();
	}
...
#pragma omp for
	...
	{
		VertexHandle handle = oldVertexHandles[i];
		newEvenVertices[handle.idx()].point = point(handle);
		if (has_vertex_texcoords2D())
			newEvenVertices[handle.idx()].tex = texcoord2D(handle);
	}
...
#pragma omp for
	...
	{
		EdgeHandle handle = oldEdgeHandles[i];
...
#pragma omp for
	...
	{
		FaceHandle handle = oldFaceHandles[i];
...
#pragma omp for
	for (unsigned int i = 0; i < edges; i++)
	{
		delete_edge(oldEdgeHandles[i], true);
	}

	// Permanently delete old vertex data
	garbage_collection();
